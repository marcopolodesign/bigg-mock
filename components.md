...
**`showAfternoon` — Mobility & recovery (2026-07-28):** `BiggDayScreen` passes `showAfternoon={isRestDay}`. It was first hidden everywhere (`false`) at the user's request, then re-scoped to rest days only, where the timeline leads with it. So: off on training days, on for Thursday. Nothing was deleted — pass `true` to bring it back everywhere.
**Rest day (2026-07-28):** driven by a new `isRestDay` prop. The day is titled by a **centred block above the timeline** — icon, title, note, all centre-aligned, over a blurred violet→blue radial gradient (`aria-hidden`, `pointer-events-none`) — not by a milestone inside it, and the "Tu BIGG day" header + filter are dropped entirely (nothing to filter). The timeline then leads with Mobility & recovery, hoisted there via a shared `mobilityMilestone` const so the JSX isn't duplicated between the rest-day slot and its normal slot. The old standalone rest-day chip in `BiggDayScreen` is deleted.
...
**"Actividad del día" card body — `cardVariant === 3` (2026-07-28):** variant 3 is the only one the app actually renders. The header pill reads **"Actividad del día"** (renamed from "Entrenamiento del día" 2026-07-28). Its card **shell is otherwise unchanged** (dark `#3d3d3d` wrapper, header + chevron, inner light card, "Bloque cambiado para esta ubicación" indicator, and the overlapping CTA block with "Donde vas a entrenar?" + Reservar clase/Iniciar entrenamiento). Only the **inner content** changed: the expandable `FlapItem` accordion rows were replaced by a Druk Wide title ("Entrenamiento de BIGG", 26px `#565656`, wraps to 2 lines — same type recipe as `ActivityCard`'s "Running pasadas" title, mixed case, no `uppercase`) plus a non-expandable 2×2 grid of small `{i+1}. {stimulus}` chips (`bg-[#ededed]`), built from `V4_BLOCKS` with the same BIGG-vs-away name swap (`blockTitles` override / `vb.away`) that was already there. `FlapItem`, `FLAP_OVERLAP` and `openFlapId` were deleted as dead code (recoverable from git history). Variants 1/2/4 remain legacy design-option comparisons from `comparacion-opciones.html`, untouched.
**Nutrición + Pasos row (2026-07-28):** the two end-of-timeline check-ins sit side by side in a single `flex flex-row items-stretch gap-[10px]` wrapper instead of stacking. Each column is `flex-1 min-w-0` rather than a hard `w-50%`, so when the active filter hides one (Sueño → neither, Nutrición → only Nutrición, Entrenamiento y actividad → only Pasos) the survivor still fills the row. Equal heights come from `items-stretch` + `flex-1` on each inner card wrapper, plus `h-full` on `NutritionEntry`'s card and `h-full` on `StepsEntry`'s inner column — without those the shorter card floats at the top of its stretched column. `StepsEntry` (props: `onCompleteDay?`; consts `STEPS_TODAY`/`STEPS_GOAL`/`STEPS_WEEK_AVG`) replaced `WindDownCard` in this row on 2026-07-28 — `WindDownCard` moved to Train's `RecommendationsCarousel`. The right column follows `showStepsContent` = `!isFutureDay && showTrainingContent` — steps are activity data (so not `showSleepContent` as Wind down was) *and* a live counter, so like the sleep/nutrition check-ins they never render on a day that hasn't happened. The row wrapper keys off the same flag so it collapses cleanly instead of leaving a half-empty row.
**Indicator + sheet pattern (2026-07-28):** both cards in this row are *only indicators*; anything explanatory lives in the `BottomSheet` they open on tap. `StepsEntry`'s card therefore shows just count / goal / progress bar / "Te faltan N" and deliberately carries **no `SourceChip`** — provenance moved into its "Detalles de pasos" sheet, which has the Garmin chip, a 4-row table (hoy / objetivo / faltan / promedio 7 días), a "¿Por qué N pasos?" explanation of how the goal is derived, a "¿De dónde salen estos datos?" note, and a "Ver mi actividad" CTA calling `onCompleteDay`. `NutritionEntry` already followed this shape (tap → log what you ate); its card was restacked to `flex-col` 2026-07-28 (icon 13→16px / question / thumb buttons on their own rows, `justify-center gap-[8px]`) so it mirrors `StepsEntry`'s internal structure and the question stops wrapping at 50% width.
⚠️ **Known pre-existing bug:** `NutritionEntry`'s "Nutrición guardada" toast is `position: fixed` but renders inside the timeline wrapper's `transform: translateY(-15px)` (from `2ac0720`), and a transformed ancestor makes `fixed` resolve against that ancestor instead of the viewport — so the toast lands beside the card rather than at the bottom of the screen. Fix would be portalling the toast out; not done, not caused by the 2026-07-28 work.
**Reserved view (rewritten 2026-07-28):** `reservedClass` used to early-return an entirely separate, simpler timeline — which meant booking a class silently dropped the "Tu BIGG day" header, the filter, `SleepEntry` and the Nutrición + Pasos row. That branch and `ReservedClassCard` are gone. A booked class now flows through the normal timeline and only the **"Actividad del día" card's contents** change: body = class type, `time · location`, `AttendeeAvatars` and the class's blocks in the same 2×2 chip grid the suggested workout uses; footer = no location picker and no "Reservar clase", instead a confirmation ("Tenés tu lugar reservado") and **Ver mi clase** → `onOpenDetail`; the header chevron routes to `onOpenDetail` too. `BiggDayScreen` seeds today with `TODAY_RESERVED_CLASS`, shared with the Reservar flow's `onConfirm` so a seeded and a user-made booking can't drift.
**`SleepEntry`** (sub-component, props: `approval`, `onApprovalChange`, `onCompleteDay?`): sleep quality check-in. **Sits below the "Actividad del día" card as of 2026-07-28** (was the first timeline item); the activity card now leads the timeline. Approval (`"approved" | "rejected" | null`) is now **controlled** — lifted to `DailyWorkoutCard`'s `sleepApproval` state (defaults to `"approved"` when `isToday`) so it can be read by `NutritionEntry`'s end-of-day recap. Its "Detalles de sueño" `BottomSheet` (Duración/Calidad rows, "Ver todos"/"Ver menos" toggle, collapsible recovery tags) now ends with a "Ver mi actividad" CTA (2026-07-21, indigo `#8b78e6` — matches the sheet's existing indigo/violet accent) that closes the sheet and calls `onCompleteDay`, reusing the same callback `NutritionEntry` uses to switch the app to the Actividad tab (threaded `DailyWorkoutCard` → `SleepEntry` at its single call site).
...
**`RECOMMENDATION_ITEMS` / `RecommendationsCarousel` (updated 2026-07-28):** **`WindDownCard`** only — Padel and Lower Body were pulled "for now" at the end of the session. `WorkoutCard` and `AgregarButton` are deliberately **kept though currently unreferenced**, so restoring the two entries is a revert rather than a rewrite. `RecommendationsCarousel` adapts when `RECOMMENDATION_ITEMS.length === 1`: the card takes the full width instead of the 76% snap width and the dot strip is hidden, otherwise it reads as a broken carousel with dead space. Both conditions key off the array length, so restoring the entries restores the swipe behaviour with no further change. (Ordering churned during the session — Wind down went last, then first; and a pass that shrank every card was tried and reverted, so the cards keep their original sizing.) `WindDownCard` moved here from `DailyWorkoutCard`'s timeline and replaced the old `SleepCard`, which was deleted — last night's sleep is still surfaced by the timeline's own `SleepEntry`. It is a 260px dark-violet card (`#241a40 → #4a2e6e`) with an "Agregar a mi día" toggle (local `added` state, violet when on).
**Derived from tomorrow's session (2026-07-28):** the card is no longer a fixed 22:00 nudge — it names tomorrow's first session and works backwards to the window that protects it. `windDownWindow(sessionTime)` subtracts `PRE_SESSION_BUFFER_MIN` (60, wake→eat→travel), then `SLEEP_TARGET_H` (8h), then `WIND_DOWN_MIN` (45) — so an 08:00 session yields **22:15–23:00**. The big numeral is the window's start hour (34px, not the 40px the old `10'` used: a HH:MM value is ~5 glyphs and at 40px it crowded the session label into truncating), the sub-label reads "MAÑANA BIGG CLASS 08:00HS", and the body states the full range. `NEXT_SESSION` is the mock stand-in for tomorrow's booked session — **set it to `null`** to exercise the no-session fallback (generic copy, `10'` numeral, and the carousel subtitle reverts to "Preparate para descansar mejor"). The subtitle is otherwise `Mañana entrenás {time}`. The three per-step routine rows were dropped 2026-07-28 to make it lighter than the workout cards beside it; content is grouped at the top with a `flex-1` spacer pinning the CTA to the bottom, so the freed space reads as air rather than a mid-card gap. `StepsCard` was deleted in the same session: it duplicated the timeline's new `StepsEntry` (same 3.200/10.000 figures one scroll apart), and the timeline is now the single place steps are shown on Train. Removing it also left `SourceChip` unused in this file.
...
**Used by:** `ActivityCard` (timeline activities — "Tomado desde …"), `DailyWorkoutCard`'s `StepsEntry` (bare "Garmin", no prefix — the 50%-wide timeline card has no room for one), and `ActividadScreen`'s `ActividadStepsCard` ("Datos de Garmin"). Train's `SleepCard` and `StepsCard` were both deleted 2026-07-28 (carousel slot taken by `WindDownCard`; steps now live in the timeline), so `BiggDayScreen` no longer imports this.
...
**Props:** `onClose: () => void`
**Purpose:** Full-screen profile screen — lightweight port of `biggapp/Screens/Profile/Profile.js` (real image-picker, i18n, sign-out/delete-account logic out of scope). Header: back nav "Volver" (same chevron SVG as `ClassDetailScreen`), circular avatar (`imgEllipse167`) with a `Pencil` edit badge overlapping bottom-right, name "Mateo", "Miembro desde 2023 · 4 amigos" line, decorative "Editar" pill. Below: `ObjetivoCard` (ported from `biggapp/Components/Objetives/Objetives.js`, "has objective" state only — dimmed photo background (`imgObjetivoPhoto`, i.e. `imgPerformanceImage`), "Objetivo" label, Druk uppercase goal title "GANAR FUERZA" + chevron, "Plan de entrenamiento: <underlined plan name>") and `MisPesosCard` (ported from `Components/Weights/Weights.js` — `Dumbbell` icon + circular chevron button + "Mis Pesos" / "Ver todos" in `#888888`). Below those: two decorative static rows (Idioma with flag emoji, Cerrar sesión in red) + app version footer — inspired by, not a 1:1 port of, the real screen's tail.
**2026-07-28 additions:**
  - `ObjetivoCard` is now a `<button>` taking `onOpen`, which mounts **`ObjetivosScreen`** (see below) behind an `AnimatePresence`. `ProfileScreen` returns a fragment so the overlay can be a sibling of the panel.
  - **`BiggFriendsCard`** (new) pairs with `MisPesosCard` — Mis Pesos moved `w-[47%]` → `flex-1 min-w-0` and the two sit in a `flex gap-[12px] items-stretch` row. Same shell, but the icon slot holds four overlapping avatar bubbles (26px, 9px overlap, white 2px ring) — the same treatment the reserved-class card uses for attendees.
  - **`BenchmarkContainer`** moved here from the Train tab, rejoining the two Activity-tail cards it was split from in the 2026-07-21 IA pass. `BiggDayScreen` keeps its `imgPerformanceImage1` import (another component still uses it); `ProfileScreen` imports the same asset as `imgBenchmarkPhoto`.
  - **`MembresiaCard`** (new) — the profile-side counterpart to `BiggDayScreen`'s `MembershipContainer`. That one is the **upsell** for non-premium users; this is the **status** card for someone who already has it (dark surface, lime "Activa" badge, BIGG PREMIUM in Druk, sede + antigüedad, next renewal, "Gestionar membresía" row). Sits after Benchmark, before the Idioma / Cerrar sesión tail.
...
### `ObjetivosScreen`
**File:** `src/app/components/ObjetivosScreen.tsx` (new 2026-07-28)
**Props:** `onClose: () => void`
**Purpose:** Opened from `ProfileScreen`'s `ObjetivoCard`. Built to a design Mateo supplied: intro copy, the primary objective as a photo hero (label / Druk title / "Plan de entrenamiento: …" / chevron), then a white card of the three measurable targets that drive the streak — **Actividad semanal** (lime-disc `Flame`, day strip L·M·M·J·V·S·D where each day toggles and the count updates live), **Sueño** (`Moon`, 8h), **Pasos diarios** (`Footprints`, 10.000) — via a shared `TargetRow`, closing with the recalculation note.
**Beyond the reference — stacking goals:** an "Otros objetivos" section with **Agregar objetivo** opening a `BottomSheet` of the remaining `TRAINING_GOALS` catalogue entries. Promoting an extra ("Hacer principal") **swaps** it with the current primary rather than overwriting, so no goal is lost; each extra also has an X to remove. All local state — there is no goals API in this repo.
**Animation:** slides in from the right, same spring as the other overlays. z-index **68** — above `ProfileScreen`'s 67.
