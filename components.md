...
**Nutrición + Pasos row (2026-07-28):** the two end-of-timeline check-ins sit side by side in a single `flex flex-row items-stretch gap-[10px]` wrapper instead of stacking. Each column is `flex-1 min-w-0` rather than a hard `w-50%`, so when the active filter hides one (Sueño → only Wind down, Nutrición → only Nutrición) the survivor still fills the row. Equal heights come from `items-stretch` + `flex-1` on each inner card wrapper, plus `h-full` on `NutritionEntry`'s card and `h-full` on `StepsEntry`'s inner column — without those the shorter card floats at the top of its stretched column. `StepsEntry` (daily step count vs. goal, progress bar, Garmin chip; `STEPS_TODAY`/`STEPS_GOAL` consts) replaced `WindDownCard` in this row on 2026-07-28 — `WindDownCard` moved to Train's `RecommendationsCarousel`. Note the right column now follows `showTrainingContent` (steps are activity data), not `showSleepContent` as Wind down did.
...
**`RECOMMENDATION_ITEMS` / `RecommendationsCarousel` (updated 2026-07-28):** Padel `WorkoutCard` → **`WindDownCard`** ("Preparate para descansar mejor") → Lower Body `WorkoutCard` — 3 items. `WindDownCard` moved here from `DailyWorkoutCard`'s timeline and replaced the old `SleepCard`, which was deleted — last night's sleep is still surfaced by the timeline's own `SleepEntry`. It is a 260px dark-violet card (`#241a40 → #4a2e6e`) with RUTINA NOCTURNA / HOY 22:00HS labels, a big `10'`, three routine rows, and an "Agregar a mi día" toggle (local `added` state, violet when on). `StepsCard` was deleted in the same session: it duplicated the timeline's new `StepsEntry` (same 3.200/10.000 figures one scroll apart), and the timeline is now the single place steps are shown on Train. Removing it also left `SourceChip` unused in this file.
