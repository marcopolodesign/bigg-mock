...
**`showAfternoon` — Mobility & recovery is currently OFF (2026-07-28):** `BiggDayScreen` passes `showAfternoon={false}`, so the "Mobility & recovery" milestone and its `AfternoonRecommendationCard` do not render on any day. This is a temporary hide at the user's request — nothing was deleted, the gate and the component are intact. Flip the prop back to `true` to restore it everywhere, or `!isToday` for today-only.
...
**`RECOMMENDATION_ITEMS` / `RecommendationsCarousel` (updated 2026-07-28):** **`WindDownCard`** ("Preparate para descansar mejor") → Padel `WorkoutCard` → Lower Body `WorkoutCard` — 3 items, Wind down **first** as of 2026-07-28. (It briefly sat last earlier the same day, and a pass that shrank every carousel card was tried and reverted — the cards keep their original sizing.) `WindDownCard` moved here from `DailyWorkoutCard`'s timeline and replaced the old `SleepCard`, which was deleted — last night's sleep is still surfaced by the timeline's own `SleepEntry`. It is a 260px dark-violet card (`#241a40 → #4a2e6e`) with an "Agregar a mi día" toggle (local `added` state, violet when on).
