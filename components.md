...
**`RECOMMENDATION_ITEMS` / `RecommendationsCarousel` (updated 2026-07-28):** **`WindDownCard`** ("Preparate para descansar mejor") → Padel `WorkoutCard` → Lower Body `WorkoutCard` — 3 items, Wind down **first** as of 2026-07-28 (it briefly sat last earlier the same day). `WindDownCard` moved here from `DailyWorkoutCard`'s timeline and replaced the old `SleepCard`, which was deleted — last night's sleep is still surfaced by the timeline's own `SleepEntry`. It is a 260px dark-violet card (`#241a40 → #4a2e6e`) with RUTINA NOCTURNA / HOY 22:00HS labels, a big `10'`, one description line and an "Agregar a mi día" toggle (local `added` state, violet when on). The three per-step routine rows were dropped 2026-07-28 to make it lighter than the workout cards beside it; content is grouped at the top with a `flex-1` spacer pinning the CTA to the bottom, so the freed space reads as air rather than a mid-card gap. `StepsCard` was deleted in the same session: it duplicated the timeline's new `StepsEntry` (same 3.200/10.000 figures one scroll apart), and the timeline is now the single place steps are shown on Train. Removing it also left `SourceChip` unused in this file.
**Carousel card sizing (2026-07-28):** all recommendation cards were scaled down toward the compact feel of the timeline's `StepsEntry`. `WorkoutCard`'s body went `h-[214px]` → `h-[150px]` with title 22→17px, exercises 13→11px (gap 6→4), badge `h-[18px]`→`h-[16px]`, clock SVG 38×67→28×50 and tighter padding; `AgregarButton` went `pt-[45px]`/`pb-[15px]`→`pt-[38px]`/`pb-[13px]` with a 24→20px icon and 15→13px label. ⚠️ The button is pulled under the card by a negative margin, so its `mb-[-34px]`→`mb-[-28px]` must stay ~10px less than the button's `pt` or the label gets clipped. `WindDownCard` was set to `h-[193px]` (its `10'` 40→30px) to match the workout cards' new total height of 150 + ~43px of visible button.
