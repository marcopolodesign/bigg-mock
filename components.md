...
**Used by:** `ActivityCard` (timeline activities — "Tomado desde …"), `StepsCard` ("Datos de Garmin"), `DailyWorkoutCard`'s `StepsEntry` (bare "Garmin", no prefix — the 50%-wide timeline card has no room for one). Train's `SleepCard` ("Datos de Apple Health", `onDark`) was deleted 2026-07-28 when `WindDownCard` took its carousel slot; `ActividadScreen` keeps its own `ActividadSleepCard`.
